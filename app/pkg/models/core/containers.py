...
class Containers:
    ...
    def __patch_container_configuration(
        ...
        dsn = conf.get(dsn_configuration_path)
        db_path = conf.get(database_configuration_path)
        test_dsn = dsn.replace(db_path, prefix + db_path)

        conf.set(dsn_configuration_path, test_dsn)
